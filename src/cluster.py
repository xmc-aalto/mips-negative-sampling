...
parser.add_argument('--dataset', type=str, required=False, default='amazoncat13k')
...
if __name__ == '__main__':
    ...
    if dataset == '670k':
        ...
        mlb = build_tree_by_level('./data/Wiki10-31K/train.txt', 
                                  ...
                                  1e-4, 4, [13], './data/Wiki10-31K/groups')
        groups = np.load(f'./data/Wiki10-31K/groups-last.npy', allow_pickle=True)
        new_group = []
        for group in groups:
            new_group.append([mlb.classes_[i] for i in group])
        ...
        mlb = build_tree_by_level('./data/Eurlex-4K/train.txt', 
                                  ...
                                  1e-4, 4, [10], './data/Eurlex-4K/groups')
        groups = np.load(f'./data/Eurlex-4K/groups-last.npy', allow_pickle=True)
        new_group = []
        for group in groups:
            new_group.append([mlb.classes_[i] for i in group])
        ...
        mlb = build_tree_by_level('./data/AmazonCat-13K/train.txt', 
                                  ...
                                  1e-4, 4, [12], './data/AmazonCat-13K/groups')
        groups = np.load(f'./data/AmazonCat-13K/groups-last.npy', allow_pickle=True)
        new_group = []
        for group in groups:
            new_group.append([mlb.classes_[i] for i in group])
